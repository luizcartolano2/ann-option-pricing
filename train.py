...
usecols = ['put_or_call', 'strike_value', 'settle_price',
           'future_price', 'tenor', 'volimp']
# read dataframe
df = pd.read_csv('data-source/options-data.csv', usecols=usecols)
# set a risk free column
df['risk_free'] = np.ones((len(df),)) * np.log(1.00501)

# read x_values and y_values
x_values = df[['put_or_call', 'strike_value', 'risk_free', 'future_price', 'tenor', 'volimp']].values
...
test_df.to_csv('data-source/test_df.csv')
...
# Train
save_model_path = f'models/train_model_at_{today_datetime}.model'
record_path = f'records/loss_{today_datetime}.txt'
save_optimizer_path = f'optimizer/optimizer_model_at_{today_datetime}.optimizer'
